...
**结论：v2 已开始开发，当前处于 `M2 Session + 旧数据迁移` 进行中。**

当前仓库已经完成 `P1.1`，并进入 `P1.2/X1`。不得再把仓库整体描述为“v2 尚未开始”；后续每次更新时都必须按本文件的状态规则同步修正总览、任务卡片和里程碑。
...
当前状态：进行中  
说明：已落地 session 目录结构、最近会话恢复、`--session`、`/sessions`、`/session <id>`、`/new` 和旧版 `memory.json` 首次迁移；但测试执行环境仍不稳定，且文档、实现与验收条目还需继续收口。
...
- 异常退出不要求补做会话结束钩子
- 当前仍不得改成 `[x]`，因为本地自动化测试链路和文档同步验证尚未完全收口
...
当前状态：进行中  
说明：已支持旧版 `memory.json` 自动迁移到 session 结构，并加入避免重复迁移的幂等保护；但“迁移失败可回滚”仍未形成明确闭环，测试也未形成稳定自动验收。
...
当前状态：进行中  
说明：已补充 session / migration / CLI 基础测试，但当前机器的文件系统权限会影响 `pytest` 稳定执行，因此不能判定该项完成。
