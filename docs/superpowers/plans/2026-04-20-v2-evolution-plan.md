...
| 1 | E2 Agent 执行安全底座 | P0 | 防止失控，保证工具调用可控、可解释 | 3.1 循环保护、4.1 参数描述、4.4 插件生命周期、7.1 结构化错误、7.2 自动重试 | E1 |
...
- [ ] 在 `AgentLoop` 中增加最大工具迭代次数，例如 `max_tool_iterations = 20`
- [ ] 增加重复工具调用检测，发现循环时注入系统提示并停止盲目执行
- [ ] 从工具 docstring 的 Args 部分提取参数描述，写入 JSON Schema
- [ ] 引入插件 manifest、插件级配置和工具命名空间
- [ ] 定义 `BasePlugin.initialize()`、`register_tools()`、`shutdown()` 生命周期
- [ ] 插件加载失败时隔离错误，不影响核心工具和其他插件
- [ ] 定义 `ToolError`、`CommandTimeoutError` 等结构化错误类型
- [ ] 为可重试错误增加有限次数自动重试和 recovery hint
- [ ] 为上述能力补充单元测试和至少一个端到端工具循环保护测试

验收标准：

- 模型连续请求相同工具时，Agent 能在上限内停止并给出明确提示
- `/tools` 或工具 schema 中能看到参数描述
- 单个插件加载失败时，Agent 仍能启动并加载其他插件
- 工具失败时返回包含错误类型和恢复建议的结果，而不是裸字符串异常
