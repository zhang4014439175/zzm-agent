...
**执行规则:** 每次只执行一个任务。执行完当前任务后必须停止，不得自动继续下一项。完成后要明确提示「当前任务已完成，下一项是 Mxx，是否执行 Mxx？」。例如检测到 `M04` 已完成时，应提示「`M04` 已完成，下一项是 `M05`，是否执行 `M05`？」；`M05` 完成后，再提示「`M05` 已完成，下一项是 `M06`，是否执行 `M06`？」。
**实现约束:** 当前版本按 MVP 执行，不要求插件热重载、不要求 session 级记忆检索。`ToolRegistry` 必须保持单一注册来源，避免全局 registry 与局部 registry 并存导致插件未注册到实际运行实例。
**安全约束:** `shell.py` 和 `file_ops.py` 仅按本地可信环境实现，不视为可直接对外暴露的生产能力。若后续扩展为 Web/API/多用户模式，必须补充命令白名单、目录访问限制、超时/资源限制和审计能力。
...
- [ ] **M07** 运行测试确认失败（可为 ImportError 或断言失败，不要求固定错误类型）
