...
3. 单独列出“开发前”和“开发后”的区别。描述可观察行为，不要只写内部组件变化。
4. 单独列出“现在已经能做到什么”和“现在还做不到什么”。不得把未来预留接口描述成已经可用的功能。
5. 第一次出现英文或技术术语时，必须立刻用一句日常语言解释。能用中文日常表达时，不使用术语。例如不能只写 deadline、LIFO、callback、token、checkpoint。
6. 不允许用类名、函数名、字段名或执行链路作为功能解释。读者即使跳过所有代码内容，也必须能理解前面至少 70% 的文档。
7. 通俗说明完成后，再增加“给开发者看的实现位置”，简要列出相关代码文件、关键类/函数和执行链路。每个代码名称后说明它具体负责什么。
8. 最后写测试覆盖和实际测试结果，并用普通语言说明这些测试证明了什么。
9. 避免空泛表达，如“提升健壮性”“完善机制”“统一能力”。如果使用，必须紧接具体说明：防止了什么现象，失败时会看到什么。
10. 全文优先使用短句。每段只表达一个重点。不要连续堆砌超过 3 个陌生术语。
